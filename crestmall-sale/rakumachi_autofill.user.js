...
// @version      1.0
// @description  楽待の売却査定フォームをクレストモールの情報で自動入力（査定トップ3項目は確実、詳細ページはラベル推定）
...
(function () {
  ...
  const D = {
    ...
    comment:
      ...
      '土地・建物を一括所有（法人名義）。抵当権は日本政策金融公庫のみで残債も小さく、権利関係はクリーンです。資料（レントロール・送金明細・公庫返済予定表・固定資産税通知・火災保険証券）完備。\n\n' +
  ...
  function fillDetail() {
    ...
    document.querySelectorAll('input, textarea').forEach(el => {
      ...
      if (['hidden', 'submit', 'button', 'file', 'checkbox', 'radio', 'password'].indexOf(type) >= 0) return;
      if (!visible(el)) return;
      const meta = metaText(el);
      if (el.tagName === 'TEXTAREA' && /備考|要望|コメント|メッセージ|その他|質問|相談|自由/.test(meta)) {
        setNative(el, D.comment); commentDone = true; n++; return;
      }
    ...
    toast(n + '項目を推定入力。送信前に必ず目視で確認してください');
  }
  ...
  function panel() {
    ...
    p.appendChild(mkBtn('このページを自動入力', run, true));
    p.appendChild(mkBtn('📋 コメントをコピー', copyComment));
    const note = document.createElement('div'); note.textContent = 'トップ3項目は確実。詳細ページは推定なので送信前に確認。';
