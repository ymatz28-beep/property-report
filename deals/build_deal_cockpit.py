...
function noiOf(key, c){
  ...
  const revenue = s.adr_yen*nights;
  const noi = revenue - revenue*s.var_opex_rate - s.fixed_annual_yen;
...
function setupCost(key, c){
  ...
  const setup = s.needs_setup ? acq.minpaku_setup_yen : 0;
  return {reno, setup};
}
...
function init(){
  ...
  ['input','change'].forEach(ev=>{
    [price,rate,years,down,occM,occR,rent,capToggle,renoRental,yk_value,yk_loan,yk_rent,yk_hold]
