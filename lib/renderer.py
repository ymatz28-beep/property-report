...
PRIVATE_NAV = [
    ...
    {"href": "/minpaku/", "label": "Minpaku"},
    {"href": "/trading/", "label": "Trading"},
    {"href": "/intel/", "label": "Insight"},
    {"href": "/health/", "label": "Health"},
    {"href": "https://iuma-family.pages.dev/", "label": "Medical"},
...
# ---------------------------------------------------------------------------
# Section Nav SSoT — sibling tabs within the same section (e.g. /bookmarks/*)
# Rule: 同一ディレクトリ下に姉妹ページが2つ以上あれば必ずここに登録。
#       新規ページ追加時は PRIVATE_NAV か SECTION_NAV の少なくとも片方必須。
# Key = URL path prefix (must end with "/"). Pages under the prefix get the tabs.
# ---------------------------------------------------------------------------
SECTION_NAV: dict[str, list[dict]] = {
    "/bookmarks/": [
        {"href": "/bookmarks/", "label": "一覧"},
        {"href": "/bookmarks/daily_brief.html", "label": "Daily Brief"},
    ],
}


def get_section_nav(current_page: str) -> list[dict]:
    """Return sibling-tab nav items for the section containing current_page.

    current_page: either full path like "/bookmarks/decide.html" or just label.
    Returns [] if no matching section (page renders without section tabs).
    """
    if not current_page:
        return []
    for prefix, items in SECTION_NAV.items():
        if current_page.startswith(prefix) or any(current_page == it["href"] or current_page == it["label"] for it in items):
            return items
    return []


def render_section_nav_html(current_path: str) -> str:
    """Return self-contained <style>+<nav> snippet for the section sibling tabs.

    Use from generators that don't go through nav.html (e.g. generators that
    build HTML via f-strings). Insert right after the <header class="site-header">
    block. Returns "" if current_path isn't in any section.
    """
    items = get_section_nav(current_path)
    if not items:
        return ""
    css = """<style>
.section-nav {
  display: flex; gap: 4px; align-items: center;
  padding: 8px 16px; overflow-x: auto; -webkit-overflow-scrolling: touch;
  background: rgba(22,24,31,0.70); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);
  border-bottom: 1px solid rgba(255,255,255,0.06);
  position: sticky; top: var(--gnav-height, 52px); z-index: calc(var(--z-nav, 100) - 1);
  scrollbar-width: none;
}
.section-nav::-webkit-scrollbar { display: none; }
.section-nav a {
  flex-shrink: 0; color: #9ca3af; text-decoration: none;
  font-size: 12px; font-weight: 500; padding: 6px 12px; border-radius: 6px;
  transition: background .15s, color .15s; white-space: nowrap;
}
.section-nav a:hover { background: rgba(255,255,255,0.065); color: #f5f5f7; }
.section-nav a[aria-current="page"] { background: rgba(99,102,241,0.18); color: #c7d2fe; }
</style>"""
    links = []
    for it in items:
        active = ' aria-current="page"' if it["href"] == current_path else ""
        links.append(f'<a href="{it["href"]}"{active}>{it["label"]}</a>')
...
def create_env(extra_dirs: list[Path] | None = None, scope: str = "public") -> Environment:
    ...
    # Section nav (sibling tabs). Template can override by passing current_page.
    env.globals["section_nav_items"] = []
    env.globals["get_section_nav"] = get_section_nav
