...
# ---------------------------------------------------------------------------
#
# Google DESIGN.md spec compatibility (Apache 2.0, github.com/google-labs-code/design.md):
#   Each entry below corresponds to a Google Token Reference:
#     "accent"     → {colors.accent}
#     "gold"       → {colors.gold}
#     "fs-h1"      → {typography.h1.fontSize}
#     "space-4"    → {spacing.md}
#   Future export to Google YAML frontmatter format is supported via this 1:1 mapping.
...
TOKENS: dict[str, str] = {
    ...
    "blue-glow": "rgba(74,144,217,0.12)",
    ...
    # Skeleton loading
    "skeleton-base": "#1a1d27",
    "skeleton-shine": "#242836",
...
/* === Table Column Width Policy (intelligent allocation) ===
   バッジ/数値/短いenum列に .col-tight を付け、内容ぴったりの自然幅まで縮める。
   自由テキスト列（案件名等）はクラス無指定のまま残り幅を吸収させる。
   table-layout:fixed とは併用しない（テキスト列を潰した過去の事故パターン。ux-lessons.md #2） */
...
.trend-up::before { content: '▲ '; font-size: 0.75em; }
.trend-down { color: var(--red); }
.trend-down::before { content: '▼ '; font-size: 0.75em; }
...
.empty-state .empty-icon { font-size: var(--space-12); margin-bottom: var(--space-4); opacity: 0.3; }
.empty-state .empty-title { font-size: var(--fs-h3); font-weight: 600; margin-bottom: var(--space-2); color: var(--text-secondary); }
.empty-state .empty-desc { font-size: var(--fs-body); }
...
/* === Badge variants === */
.badge { display: inline-flex; align-items: center; padding: 2px 8px; border-radius: 9999px; font-size: var(--fs-xs); font-weight: 600; }
.badge-green { background: rgba(34,197,94,0.15); color: var(--green); }
.badge-red { background: rgba(239,68,68,0.15); color: var(--red); }
.badge-yellow { background: rgba(234,179,8,0.15); color: var(--yellow); }
.badge-blue { background: rgba(59,130,246,0.15); color: var(--blue); }
.badge-muted { background: rgba(124,130,147,0.15); color: var(--text-muted); }

/* === Light theme override (opt-in via data-theme="light", see get_theme_toggle_css) === */
:root[data-theme="light"] {
  ...
  --text-muted: #6b7080;
  --text-decorative: #9098ab;
  --gold: #a8842e;
  --gold-dim: rgba(168,132,46,0.12);
  --accent: #4f46e5;
  --accent2: #7c3aed;
  --green: #16a34a;
  --red: #dc2626;
...
/* badge-* は暗背景前提の固定rgbaのため、ライトテーマでは明背景用に個別上書き */
:root[data-theme="light"] .badge-green { background: rgba(22,163,74,0.12); }
:root[data-theme="light"] .badge-red { background: rgba(220,38,38,0.12); }
:root[data-theme="light"] .badge-yellow { background: rgba(161,98,7,0.12); }
:root[data-theme="light"] .badge-blue { background: rgba(37,99,235,0.12); }
:root[data-theme="light"] .badge-muted { background: rgba(107,112,128,0.12); }

/* === Global theme toggle (floating button, see base.html) === */
.theme-toggle-fab {
  position: fixed;
  right: 16px;
  bottom: 16px;
...
@media (max-width: 640px) {
  .theme-toggle-fab { right: 10px; bottom: 10px; padding: 6px 10px; font-size: 11px; }
}
