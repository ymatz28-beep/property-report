...
<div style="background:#fff;border:1px solid #e5e7eb;border-left:4px solid #dc2626;border-radius:8px;padding:14px 16px;margin:10px 0">
  <div style="font-weight:700;font-size:16px;color:#111827;line-height:1.4;overflow-wrap:anywhere;line-break:strict">{label}</div>
  ...
  <pre style="background:#f9fafb;color:#991b1b;font-family:ui-monospace,SFMono-Regular,Consolas,monospace;font-size:12px;padding:10px 12px;margin:10px 0 0 0;border-radius:6px;white-space:pre-wrap;overflow-wrap:anywhere;line-height:1.5;max-height:200px;overflow-y:auto">{err[:500]}</pre>
    ...
    ctas = f"""
<a href="{_mailto('1', 'fix')}"    style="{btn_style};background:#16a34a;color:#ffffff">1 ・ Claude Code で修正を開始（推奨）</a>
...
<p style="color:#6b7280;margin:0 0 18px 0;font-size:13px;line-height:1.5;overflow-wrap:anywhere">
  {start:%Y-%m-%d %H:%M} JST・{elapsed:.0f}秒・<strong style="color:#111827">{ok_count}/{step_count}</strong> 成功・新規{len(diff.get('new',[]))}件
</p>
